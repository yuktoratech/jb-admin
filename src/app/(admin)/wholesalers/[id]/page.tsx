...
export default function WholesalerDetail() {
  ...
  if (error || !item)
    return (
      <ErrorState
        message={error || "Wholesaler not found."}
        onRetry={() => void load()}
      />
    );
  ...
  const columns: Array<TableColumn<Retailer>> = [
    ...
    { key: "email", header: "Email", render: (retailer) => retailer.email },
    {
      key: "phone",
      header: "Phone",
      render: (retailer) => retailer.phone || "—",
    },
    {
      key: "discount",
      header: "Discount",
      render: (retailer) => `${retailer.discountPercent}%`,
    },
    {
      key: "status",
      header: "Status",
      render: (retailer) => (
        <Badge tone={retailer.status === "active" ? "active" : "inactive"}>
          {retailer.status}
        </Badge>
      ),
    },
  ...
  const toggle = async () => {
    ...
    setNotice(null);
    try {
      const saved = await wholesalerApi.updateStatus(
        id,
  ...
  const permanentlyDelete = async () => {
    ...
    setNotice(null);
    try {
      await wholesalerApi.permanentlyDelete(id);
      router.push("/wholesalers");
    } catch (requestError) {
      setNotice(
        getApiErrorMessage(
          requestError,
          "Unable to permanently delete Wholesaler.",
        ),
      );
  ...
  return (
    <div className="mx-auto max-w-[1400px]">
      <PageHeader
        title={item.name}
        description="Wholesaler account details and owned Retailers."
        actions={
          <>
            <Button
              variant="secondary"
            ...
            >
              {editing ? "Cancel Edit" : "Edit Wholesaler"}
            </Button>
            <Button
            ...
            <Button variant="danger" onClick={() => setPermanentConfirm(true)}>
              Delete Permanently
            </Button>
          </>
        }
      ...
      {notice ? (
        <div
          role="status"
          className="mb-5 rounded-md border border-neutral-200 bg-white p-3 text-sm"
        >
          {notice}
        </div>
      ) : null}
      {editing ? (
        <section className="mb-6 rounded-lg border bg-white p-5 sm:p-6">
          <h2 className="mb-5 font-semibold">Edit supported account fields</h2>
          <WholesalerForm
            initial={item}
            onSaved={(saved) => {
              setItem(saved);
              setEditing(false);
              setNotice("Wholesaler details updated.");
            }}
          />
        </section>
      ) : null}
      ...
      <section className="rounded-lg border bg-white">
        ...
        <dl className="grid gap-5 p-5 sm:grid-cols-2 lg:grid-cols-4">
          ...
          <Info label="Retailers" value={String(retailerTotal)} />
      ...
      <section className="mt-6 overflow-hidden rounded-lg border bg-white">
        <div className="flex justify-between border-b p-5">
          <div>
            <h2 className="font-semibold">Owned Retailers</h2>
            <p className="mt-1 text-xs text-neutral-500">
              Showing the newest {retailers.length} of {retailerTotal}. This is
              a read-only Admin preview.
            </p>
          </div>
          <Link
            href={`/retailers?wholesalerId=${id}`}
            className="text-sm font-semibold text-[#7A1F2B]"
          >
            View all
          </Link>
        </div>
        {retailers.length ? (
          <Table
            columns={columns}
            rows={retailers}
            rowKey={(retailer) => retailer._id}
          />
        ) : (
          <EmptyState
            title="No Retailers"
            description="This Wholesaler has not created any Retailers."
          />
        )}
      </section>
      ...
      <ConfirmDialog
        isOpen={confirm}
        title={`${item.status === "active" ? "Deactivate" : "Activate"} Wholesaler?`}
        description={
          item.status === "active"
            ? "The account will no longer be able to sign in. Pending Retailer orders may block this action."
            : "The account will regain access."
        }
        confirmLabel={item.status === "active" ? "Deactivate" : "Activate"}
      ...
      />
      <ConfirmDialog
        isOpen={permanentConfirm}
        title="Permanently delete this wholesaler?"
        description="This action cannot be undone. Permanent deletion is only allowed when the wholesaler has no related Retailers or order history."
        confirmLabel="Delete Permanently"
        tone="danger"
        isConfirming={busy}
        onClose={() => !busy && setPermanentConfirm(false)}
